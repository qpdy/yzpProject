...
- [hash 模式和 history 模式的区别？](#68-hash-模式和-history-模式的区别)
...
## 68. hash 模式和 history 模式的区别？

### 什么是 SPA 路由

SPA（Single Page Application，单页应用）只有一个 HTML 页面，通过 JavaScript 动态更新页面内容。路由的作用就是管理 URL 与视图的对应关系。

前端路由有两种主流实现模式：**hash 模式** 和 **history 模式**。这两个模式使用的都是**浏览器原生 API**，不是任何前端框架（React / Vue / Angular）特有的，框架只是对其进行了封装。
...
- 项目简单，对 SEO 无要求
- 静态文件部署，没有服务器控制权（如 GitHub Pages、静态托管）
- 需要兼容 IE8+
- 快速原型开发
...
// 根据环境切换
const router = process.env.USE_HASH
...
#### Q10: 如何解决 history 模式刷新 404 的问题？

除了上面提到的服务器配置外，还有两个思路：

1. **静态导出（SSG）**：用 Next.js / Nuxt.js 在构建时预渲染所有路由
