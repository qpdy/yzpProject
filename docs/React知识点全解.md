...
- 内部又分为三个子阶段（Before Mutation → Mutation → Layout），每个子阶段都会遍历一次 Fiber 树，但整体是同步的：
  - **Before Mutation**：读取 DOM 快照（如 class 组件的 `getSnapshotBeforeUpdate`）；同时在此阶段开始前，React 已经通过 scheduler（底层用 `MessageChannel`）调度好了 passive effect（useEffect）的执行
...
- useEffect 的**调度**在 commit 阶段开始时就已发起（scheduleCallback），但**真正执行**发生在 commit 完成、浏览器 Paint 之后（异步）
- 在下一次渲染的 `useEffect` 回调执行之前，会先同步执行上一次的清理函数（清除订阅、定时器等），然后再执行本次 `useEffect` 回调；组件卸载时也会执行清理函数
...
- 拆分多个 Context（按更新频率）。
- `useMemo` 包裹 value。
- 分离“稳定 API”和“变化数据”到两个 Provider。
- 用 `useContextSelector`（zustand / 社区库）做字段级订阅。
- 数据层改用 Zustand 等按需订阅库替代 Context。
...
- **自动批处理（React 18）**：在事件、`Promise`、`setTimeout`、原生事件中多次 `setState` 自动合并成一次渲染，减少不必要的重渲染。
...
| 官方推荐 | ✅ React 官方推荐 | ❌ 停滞，不兼容 hooks 新特性好 |
| 关注点 | 行为/可访问性 | 实现细节 |
